...
'''
Module to override the DFT code settings with specific options
for the preliminary screening or the final relaxation
'''
...
def override_dft_settings(dftsettings : dict,
                          program : str,
                          calc_type : str):
    '''
    Overrides the DFT code settings with specific options
    for the preliminary screening or the final relaxation

    Args:
    - dftsettings: dictionary containing the DFT code settings
    - program: the DFT code program ('vasp', 'espresso')
    ...
    if calc_type == 'screening':
        if program == 'espresso':
            ...
            dftsettings['control'].update({'etot_conv_thr': dftsettings['etot_conv_thr_screening']})
            dftsettings['control'].update({'forc_conv_thr': dftsettings['forc_conv_thr_screening']})
            ...
            for i, line in enumerate(s):
                if 'EDIFFG' in line:
                    s[i] = f"EDIFFG = {dftsettings['ediffg_screening']}"
                    missing_ediffg = False
                if 'IBRION' in line:
                    missing_ibrion = False
            if missing_ediffg: s.append(f"EDIFFG = {dftsettings['ediffg_screening']}")
